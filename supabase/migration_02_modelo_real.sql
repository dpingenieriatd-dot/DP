-- =========================================================================
-- Migración 02 — Ajustar el modelo de Gestión al motor financiero REAL
-- encontrado en los datos históricos del Anexo 2 (Panel_DP_Proyectos_
-- Control_Presupuestal.html), verificado al centavo contra 15 presupuestos
-- reales de D&P.
--
...
drop policy if exists "gestion: acceso por módulo" on presupuestos;
create policy "gestion: acceso por módulo" on presupuestos
  for all using (has_module('gestion')) with check (has_module('gestion'));

drop policy if exists "gestion: acceso por módulo" on presupuesto_costos;
create policy "gestion: acceso por módulo" on presupuesto_costos
  for all using (has_module('gestion')) with check (has_module('gestion'));
